...
| **Load and capacity** | Measured on one host | `scripts/measure_load.py` ramps read concurrency and drains a queue of OCR jobs. Reads peak at about 156 requests per second at 2 threads, p50 14 ms and p95 19 ms, and beyond that throughput is flat while p50 latency climbs to 129 ms at 16 threads. The worker completes 17 jobs a minute at 3.6 seconds each, bounded by `max_jobs = 2`, which `app/worker/main.py` sets deliberately because OCR is CPU-bound. These are a capacity baseline for one machine and one dataset, not a published benchmark. Re-running on the same host while other containers were busy moved the peak between 111 and 156 requests per second, which is the honest width of the figure. What is still missing is a sustained soak test and a figure for concurrent evidence upload from many districts at once. |
| **A general API rate limit** | Declared, not enforced | Found by measuring: 4,153 read requests across several sessions produced zero 429 responses. `API_PER_SESSION` is defined in `security/ratelimit.py` at 600 requests per 60 seconds and referenced nowhere else, so it protects nothing; `LISTING_FETCH_PER_USER` is the same, though that one belongs to the listing fetcher which is deliberately absent. Both are now annotated as unwired in the source. Switching `API_PER_SESSION` on as written would cap a session at 10 requests per second against a measured capacity of 111 to 156, so it needs a number derived from measurement first. `docs/THREAT_MODEL.md` already states that rate limits protect specific endpoints and that there is no general protection, so the documentation was accurate and only the source was misleading. |
| **Automatic text-region detection** | Built, measured, not shipped | The row above predicted that isolating regions would improve the *state* rather than the extraction rate, by finding a declaration label whose value cannot be read. Measured over all 14 photographs, that prediction is wrong. A morphological region detector, gradient then a wide closing then contours filtered on aspect and area, was compared against the whole-image pass. Labels read: **10 either way**. The whole-image pass already finds every label the regions find, so the better state never materialises. Regions alone are worse, 6 labels against 10, because the detector loses text the single pass catches. The union of both reads net quantity from **1 of 14** where the whole-image pass reads 0, which is the first strict read of a declared quantity from a real photograph in this project. The knee is page segmentation rather than box count: capped at 14 boxes, psm 7 alone gains nothing, psm 6 and 7 together gain the quantity, and raising the cap to 60 adds only time. Cost is about twice the OCR time per image, which would take the worker from 17 jobs a minute to roughly 9. One extra quantity read out of 14, no better labels, for half the throughput measured in `measure_load.py` is not a trade worth making automatically, so it is recorded rather than merged. A future attempt should start from the knee: psm 6 on isolated phrase-shaped boxes is what works. |
| **Where the OCR gain already comes from** | Measured | `imaging.make_ocr_input` upscales to a 1600 pixel minimum edge and applies CLAHE before any reading happens, and that pass is doing more than any profile choice. Measured on the 14 photographs after that preparation: adding a sparse-segmentation read changes the declarations located not at all, 16 either way, 6 of 14 photographs either way. The same measurement taken on the raw files, before preparation, suggested a large gain and was misleading. Anyone tuning OCR here should measure after `make_ocr_input`, not on the file. Escalation to `sparse` is kept because a read returning almost nothing is worth a second attempt, but it is not the reason extraction works. |
...
| **Second factor** | Implemented, TOTP | RFC 6238 time-based codes, verified against the six SHA-1 test vectors the RFC publishes rather than against a library's say-so. Enrolment is two steps so a failed scan cannot lock an account out of its own factor, and `mfa_enabled` stays false until a code proves the officer holds the secret. A correct password alone returns 401 `mfa_required` with a five-minute signed challenge and sets no cookie. Removal needs the password **and** a current code, so a borrowed session cannot quietly strip the protection off an account. Recovery is administrator-mediated rather than by recovery codes: codes have to be delivered somewhere and this deployment has no mail service, so an administrator who already holds `user.reset_password` clears the factor with a mandatory reason, audited as `mfa.reset_by_administrator`. What is missing: the secret is stored unencrypted in `users.mfa_secret`, so a database read discloses it, and there is no rate limit on code attempts beyond the per-address and per-account sign-in budgets the challenge shares. |
| **SSO** | Architecture only | `external_subject` exists on the `User` model and the session layer would accommodate it. No OIDC flow exists. |
