...
| **Reading a declaration off a real pack** | Weak, and Tesseract is the constraint | Diagnosed on one photograph where the panel is plainly legible. A crop of a Tata Salt pouch, 1897 by 383, shows "NET QUANTITY: 1 kg" in large bold type with a grayscale range of 4 to 196. All seven configured OCR profiles fail on it: `default`, `line`, `english`, `hindi` and `numeric` return zero words, `sparse` returns 6 words at 0.41 confidence and `block` returns 12 at 0.57, none of them the declaration. Tesseract 5.3.0 invoked directly returns empty text at psm 3 and garbage at psm 6, so this is the engine rather than the configuration. Preprocessing narrows the gap without closing it: CLAHE then psm 6 yields "anTITY: 140" and adaptive Gaussian thresholding yields "MErQUANTi 1)". Curved glossy film with dark brown ink on a saturated orange substrate, JPEG compressed, is past what this engine does. A different reader, or a scale reference in frame, is the fix; tuning the profiles is not. |
